...
var defaultJSON = {"apikey":"","cityid":"false","username":"픽셀님","tempunit":"metric","locale":"ko-kr","textcolor":"#ffffff","textsize":"25.5","iconcolor":"default","iconsize":"24","font":"Product Sans","fontbold":"Product Sans Medium","previewmode":"true","previewsize":"large","refreshview":"false","greeting1":"좋은 아침입니다","greeting2":"좋은 점심입니다","greeting3":"좋은 오후입니다","greeting4":"좋은 저녁입니다","greeting5":"주무실 시간입니다","greeting0":"PX3에 오신 것을 환영합니다","dateformat":"MMMM d일 EEEE","quotemode":"false","refreshrate":"90","hideb":"false","event":"true","bColor":"#fff"}
...
async function updateCode(newVer){
  let widgetCode = await new Request("https://raw.githubusercontent.com/unvsDev/pixel-widget/main/Pixel%20Widget.js").loadString()
  let launcherCode = await new Request("https://github.com/unvsDev/pixel-widget/raw/main/Pixel%20Launcher.js").loadString()
...
async function showMainPanel(){
  ...
  if(suggestionsData == "1"){ enableSuggestions = true }
  }
  ...
  function loadAllRows() {
    ...
    if(latestVersion != version){
      ...
      updateBt.onTap = async () => {
        ...
        let response = await upAlert.present()
    ...
    let menuOptions = ["일반 설정", "레이아웃 설정", "배경화면 설정", "하위 요소 편집", "기타 설정"]
    for(i in menuOptions) {
      let option = new UITableRow()
      option.height = 60
      option.addText(menuOptions[i])
...
async function showLayoutPanel () {
  ...
  function loadAllRows() {
    let settingsTitle = new UITableRow()
  ...
  "Silk": "https://user-images.githubusercontent.com/63099769/109473605-d91b1780-7ab6-11eb-853d-68e07e955d1c.jpeg",
    ...
    let photo = new UITableRow()
    photo.height = 380
    let img = UITableCell.imageAtURL(scSample[layoutData.layout])
    
    photo.addCell(img)
    settings.addRow(photo)
    
    let mode = new UITableRow()
    mode.height = 70
    
    let mtitle = UITableCell.text("메인 레이아웃")
    mtitle.leftAligned()
    mode.addCell(mtitle)
    
    let mtitle2 = UITableCell.text(layoutData.layout)
    mtitle2.leftAligned()
    mtitle2.titleFont = Font.boldMonospacedSystemFont(16)
    mtitle2.titleColor = new Color("#e9897e")
    mode.addCell(mtitle2)
    
    let current = UITableCell.button("편집")
    current.rightAligned()
    mode.addCell(current)
    
    current.onTap = async () => {
      
        let moder = new UITable()
    ...
    padding.onSelect = async () => {
      ...
      alert.addTextField(layoutData["padding"][layoutData.layout].toString(), layoutData["padding"][layoutData.layout].toString())
      alert.addAction("확인")
...
let evSample = {
  ...
  "Silk": "https://user-images.githubusercontent.com/63099769/116784447-30e1f900-aacf-11eb-8d47-10e32ae94942.jpeg"
    ...
    let photo2 = new UITableRow()
    photo2.height = 380
    let img2 = UITableCell.imageAtURL(evSample[layoutData.event])
    
    photo2.addCell(img2)
    settings.addRow(photo2)
    
    let mode2 = new UITableRow()
    mode2.height = 70
    
    let m2title = UITableCell.text("이벤트 레이아웃")
    m2title.leftAligned()
    mode2.addCell(m2title)
    
    let m2title2 = UITableCell.text(layoutData.event)
    ...
    let current2 = UITableCell.button("편집")
    ...
    padding2.onSelect = async () => {
      let alert = new Alert()
      alert.title = "상단 여백을 설정하세요"
      alert.addTextField(layoutData["padding2"][layoutData.event].toString(), layoutData["padding2"][layoutData.event].toString())
      alert.addAction("확인")
      alert.addCancelAction("취소")
      let response = await alert.present()
...
async function showTeaserLayout(name, image){
  ...
  let photo = new UITableRow()
  photo.height = 380
  let img = UITableCell.imageAtURL(image)
    
  photo.addCell(img)
  teaserPanel.addRow(photo)
  
  let text = new UITableRow()
  text.height = 70
  text.isHeader = true
...
async function showWallPanel() {
  ...
  var wallData = JSON.parse(fm.readString(wallPath))
  ...
  function loadAllRows() {
    ...
    let mtitle = UITableCell.text("배경 모드")
    mtitle.leftAligned()
    mode.addCell(mtitle)
    
    let left = UITableCell.button("⬅️")
    let current = UITableCell.button(wallOption[wallData.mode])
    let right = UITableCell.button("➡️")
    ...
    current.onTap = async () => {
      let selector = new Alert()
      selector.title = "배경화면 모드 선택"
      for(thing in wallOption){
        selector.addAction(wallOption[thing])
      }
      let response = await selector.present()
      wallData.mode = response
      refreshAllRows()
    }
    ...
    if(wallData.mode == 1){
      ...
      input.dismissOnSelect = false
      input.height = 70
      var lStr = fm.fileExists(imgPath) ? "✅ 설정되었습니다" : "❌ 설정되지 않았습니다"
      input.addText("기본 배경화면 지정", lStr)
      settings.addRow(input)
      
      input.onSelect = async () => {
        let dir = await DocumentPicker.open(["public.image"])
        var imgPath = fm.joinPath(fDir, "light.png")
      ...
      let input2 = new UITableRow()
      input2.dismissOnSelect = false
      input2.height = 70
      var dStr = fm.fileExists(imgPath2) ? "✅ 설정되었습니다" : "❌ 설정되지 않았습니다"
      ...
      input2.onSelect = async () => {
        ...
        let response = await inAlert.present()
        if(response) {
          fm.remove(imgPath2)
          refreshAllRows()
        } else {
          let dir = await DocumentPicker.open(["public.image"])
          var imgPath = fm.joinPath(fDir, "dark.png")
    ...
    if(wallData.mode == 2){
      ...
      input.onSelect = async () => {
        ...
        let response = await bm.present()
        if(response != -1) {
          let text = bm.textFieldValue(0)
          wallData.book1 = text
        }
        refreshAllRows()
      }
    ...
    if(enableSuggestions && wallData.mode == 3){
      var basePath = fm.joinPath(fDir, "base.png")
      var imgPath = fm.joinPath(fDir, "light.png")
      var imgPath2 = fm.joinPath(fDir, "dark.png")
...
async function showPluginPanel(){
  ...
  minimemo.onSelect = async () => {
    let pAlert = new Alert()
    pAlert.title = "메모콘 편집"
    pAlert.message = (progData.minimemo.length < 1 ? "미니 메모가 비활성화되어 있습니다." : "미니 메모가 \"" + progData.minimemo + "\"로 설정되어 있습니다.") + "\n텍스트 필드를 비워두시면 비활성화됩니다."
  ...
  minidday.onSelect = async () => {
    ...
    pAlert.message = (progData.minidday[0].length < 1 ? "디데이 기능이 비활성화되어 있습니다." : progData.minidday[1] + ": " + progData.minidday[0]) + "\n텍스트 필드를 비워두시면 비활성화됩니다."
  ...
  miniphoto.onSelect = async () => {
    let pAlert = new Alert()
    pAlert.title = "포토콘 편집"
    pAlert.message = (progData.photo.length < 1 ? "포토콘이 비활성화되어 있습니다." : "포토콘이 파일 북마크 \"" + progData.photo + "\"(으)로 설정되어 있습니다.") + "\n\n- 사진의 파일 북마크 입력\n- 사진 크기 입력 (숫자)\n- 사진 테두리 크기 입력 (숫자)\n- 사진 테두리 색 입력 (Hex 값)"
...
async function showTeaser(message, url, term, short){
  ...
  let img = UITableCell.imageAtURL(url)
...
async function showSettingPanel() {
  ...
  shortcut.onSelect = async () => {
    ...
    var sData = ["Scriptable 앱 실행", "픽셀 위젯 실행", "픽셀 런처 실행", "네이버 QR 체크인", "카카오 QR 체크인"]
    var lData = ["scriptable://", "scriptable:///run/Pixel%20Widget%20v" + version, "scriptable:///run/Pixel%20Launcher%20v" + version, "naversearchapp://inappbrowser?url=https://nid.naver.com/login/privacyQR", "kakaotalk://con/web?url=https://accounts.kakao.com/qr_check_in"]
    for(var i = 0; i<sData.length; i++){
      var bt = new UITableRow()
      bt.height = i ? 55 : 70
  ...
  option2.onSelect = async () => {
    ...
    if(response == -1) {await settings.present()}
    else {
      let upAlert = new Notification()
...
for(thing in optionName){
  // Settings List
  var isBoolValue = (prefData[thing] != "true" && prefData[thing] != "false") ? false : true
  let option = new UITableRow()
  option.dismissOnSelect = false
  if(thing == "apikey"){
    option.addText(optionName[thing])
  } else if(thing == "cityid"){
  ...
  option.onSelect = async (number) => {
    ...
    if(n == 1 || (val != "true" && val != "false")){
      ...
      var response = await editAlert.present()
      if(response != -1){
