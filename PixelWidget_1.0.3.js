...
// Pixel Widget 1.0.3 by Xkfdhrwhdk
...
// Widget 자체 설정 값
const bnumber = 1003; // DO NOT EDIT THIS AREA
...
if(!config.runsInWidget){ // Bulit-in PWL
  ...
  let menuOptions = ["수동 새로고침", "Pixel Widget 설치", "전용 폰트 설치하기", "투명 위젯 만들기", "크레딧", "공식 디스코드"]
  ...
  if(response == 0){
    ...
    Safari.openInApp("https://github.com/xkfdhr/pixel-widget/releases", false)
    return 0
  } else if(response == 2) {
...
if(COVID_MODE) {
// Source
  const COVID_SOURCE = pl.covidsource();
  let covid_count = 0; let covid_date = "";
  if(COVID_SOURCE == "official") {
    ...
    covid_count = parseInt(covid.count.domestic) + parseInt(covid.count.overseas)
    covid_date = covid.date.replace(/\(|\)/g, '').split(',')[0]
    
  } else if(COVID_SOURCE == "live") {
  ...
  `, true)
    covid_count = parseInt(covid_live.count.replace(/명/g, ''))
    covid_date = covid_live.date
...
// Layout
  ...
  let covidText;
  if(COVID_SOURCE == "official") {
    covidText = cStack.addText('COVID-19 Alert : ' + covid_count.toString() + '명 / ' + covid_date.toString())
  } else {
    covidText = cStack.addText('CORONA Live : ' + covid_count.toString() + '명 / ' + covid_date.toString())
  }
